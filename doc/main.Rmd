...
Even though our F1 accuracy did not come out as optimistic as we would had liked it to be, we believe that this could more to do with our lack of capacity to set the parameters to run as we would wish to, such as set J to run in each level of N until it converges (i.e Nhat = NStar), and to run more levels of N. Our code allows for this just that we could not run it as such as it would had taken too much time given that R is not the best with loops and iterations.  In all, it was indeed a very illuminating experience analyzing this paper. 

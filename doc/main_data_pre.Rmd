...
```{r}
overlap_data = create_overlap(Train[[1]][,1])
```  

### compute TF-IDF of paper title  

```{r}
it_train <- itoken(Train[[1]]$Paper,
...
## Feature Reduction  
```{r}
# Use PCA to choose most important features
PCA_paper = prcomp(paper_data, center = T, scale. = T)
require("graphics")
screeplot(PCA_paper,npcs =270,type = "lines")
paper_data = PCA_paper$x[,1:270]

PCA_journal = prcomp(Journal_data, center = T, scale. = T)
require("graphics")
screeplot(PCA_journal,npcs =180,type = "lines")
Journal_data = PCA_journal$x[,1:178]
```  
### combine all the features  

```{r}
Fea_wol <- cbind(overlap_data,paper_data,Journal_data) 
Fea_wl = cbind(label = unlist(Train[[1]]$AuthorID),Fea_wol)
```  
...
result_hclust <- cutree(h,length(unique(Train[[1]]$AuthorID)))
end.time <- Sys.time()

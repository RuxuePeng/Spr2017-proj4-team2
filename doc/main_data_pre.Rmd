...
```{r}
start.time <- Sys.time()
init_cluster = kmeans(Fea_wl,centers = 100,nstart =5)
end.time <- Sys.time()
end.time-start.time
```
...
## Compute predicitons matrix of all the partitioning in same level and their F1s scorel  

```{r}
#F1(one_data = Data[[1]], a_c = a_c,b_d = b_d) return a list where 1st element is F1vec, 2nd is labels matrix, 3rd is the partitioning models it tried
F1_result = F1(one_data = Data[[1]])
F1score = F1_result[[1]]
labels = F1_result[[2]]
```
...
# Updating Lamda for each level after obtain F1 and score
# extract the best score and its F1
# ind_N_star <- which(score==max(score))
# ind_N_hat <- which(F1==max(F1))
# if(ind_N_star!=ind_N_hat){
#   label_i <-labels[,ind_N_star]
#   label_j <- labels[,ind_N_hat]
#   Fea_wol_star <- data.frame(cbind(Fea_wol,label_1=label_i))
#   Fea_wol_hat <- data.frame(cbind(Fea_wol,label_1=label_j))
#   F_T_star <- rowSum(aggregate(Fea_wol_star,by=list(Fea_wol_star$label_1),FUN=mean))
#   F_T_hat <- rowSum(aggregate(Fea_wol_hat,by=list(Fea_wol_hat$label_1),FUN=mean))
#   lamda_new <- lamda + F_T_star - F_T_hat
# }else{
# lamda_new <- lamda
# }
